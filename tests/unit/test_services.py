...
def test_can_get_book_by_author(in_memory_repo):
    assert str(find_book_services.get_book_by_title('HaoChen Li', in_memory_repo)) == 'None'
    assert str(find_book_services.get_book_by_title('Sherlock Holmes: Year One',
                                               in_memory_repo)) == '[<Book Sherlock Holmes: Year One, book id = 11827783>]'


def test_can_get_book_by_publisher(in_memory_repo):
    assert str(find_book_services.get_book_by_publisher('HaoChen Li', in_memory_repo)) == 'None'
    assert str(find_book_services.get_book_by_publisher('Dynamite Entertainment',
                                                   in_memory_repo)) == '[<Book Sherlock Holmes: Year One, book id = 11827783>]'


def test_can_get_book_by_release_year(in_memory_repo):
    assert str(find_book_services.get_book_by_release_year(273648237, in_memory_repo)) == 'None'
    assert str(find_book_services.get_book_by_release_year(2011,
...
def test_can_get_book_by_ebook_status(in_memory_repo):
    assert str(find_book_services.get_book_by_ebook_status(True,
...
def test_can_get_book_by_number_of_pages(in_memory_repo):
    assert str(find_book_services.get_book_by_number_of_pages(144, in_memory_repo)) == '[<Book War Stories, Volume 4, book id = 27036539>, <Book Sherlock Holmes: Year One, book id = 11827783>]'
    assert str(find_book_services.get_book_by_number_of_pages(27836478236, in_memory_repo)) == 'None'
