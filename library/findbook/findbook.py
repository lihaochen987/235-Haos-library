...
def initialise_forms():
    return IdSearchForm(), AuthorSearchForm(), PublisherSearchForm(), ReleaseYearSearchForm()
...
def find_book():
    id_form, author_form, publisher_form, release_form = initialise_forms()
...
def find_book_by_release_year():
    ...
    if release_form.validate_on_submit():
        if (services.get_book_by_release_year(release_form.book_year.data, repo.repo_instance) == None):
            return render_page(id_form, author_form, release_form, release_form)
        else:
            return render_template(
                'findbook/displaybooks.html',
